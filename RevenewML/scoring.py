...
def main(database, dsn):
    ...
    duplicate_reports_query = open(application_path + '/RevenewML/preprocessing/sql/duplicate_reports.sql').read().format(
    ...
    df_output.to_sql(name=table_name, con=engine, if_exists='replace', schema=schema, index=False)
